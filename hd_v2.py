...
st.sidebar.caption("Filter specimens by country and/or province/state. Leave empty to include all.")

# ── Country filter ────────────────────────────────────────────────────────────
country_search = st.sidebar.text_input("🔍 Search countries", placeholder="e.g. Canada")
if country_search:
    country_matches = [c for c in all_countries if country_search.lower() in c.lower()]
    if country_matches:
        to_add_countries = st.sidebar.multiselect(
            f"Countries ({len(country_matches)} found)",
            options=country_matches,
            default=[],
            key="country_search_results"
        )
        cc1, cc2 = st.sidebar.columns(2)
        if cc1.button("➕ Add", use_container_width=True, key="add_countries"):
            st.session_state.selected_countries = sorted(
                list(set(st.session_state.selected_countries + to_add_countries))
            )
            st.rerun()
        if cc2.button("➕ All", use_container_width=True, key="add_all_countries"):
            st.session_state.selected_countries = sorted(
                list(set(st.session_state.selected_countries + country_matches))
            )
            st.rerun()
    else:
        st.sidebar.caption("No countries match that search.")

if st.session_state.selected_countries:
    st.sidebar.markdown(f"**Countries:** {len(st.session_state.selected_countries)} selected")
    st.sidebar.caption(", ".join(st.session_state.selected_countries))
    if st.sidebar.button("❌ Clear countries", use_container_width=True):
        st.session_state.selected_countries = []
        st.rerun()
else:
    st.sidebar.markdown("**Showing all countries**")

# ── Province filter ───────────────────────────────────────────────────────────
# Dynamically limit province options based on selected countries
if st.session_state.selected_countries:
...
province_search = st.sidebar.text_input("🔍 Search provinces/states", placeholder="e.g. British Columbia")
if province_search:
    province_matches = [p for p in available_provinces if province_search.lower() in p.lower()]
    if province_matches:
        to_add_provinces = st.sidebar.multiselect(
            f"Provinces/States ({len(province_matches)} found)",
            options=province_matches,
            default=[],
            key="province_search_results"
        )
        pc1, pc2 = st.sidebar.columns(2)
        if pc1.button("➕ Add", use_container_width=True, key="add_provinces"):
            st.session_state.selected_provinces = sorted(
                list(set(st.session_state.selected_provinces + to_add_provinces))
            )
            st.rerun()
        if pc2.button("➕ All", use_container_width=True, key="add_all_provinces"):
            st.session_state.selected_provinces = sorted(
                list(set(st.session_state.selected_provinces + province_matches))
            )
            st.rerun()
    else:
        st.sidebar.caption("No provinces/states match that search.")

if st.session_state.selected_provinces:
    st.sidebar.markdown(f"**Provinces/States:** {len(st.session_state.selected_provinces)} selected")
    st.sidebar.caption(", ".join(st.session_state.selected_provinces))
    if st.sidebar.button("❌ Clear provinces", use_container_width=True):
        st.session_state.selected_provinces = []
        st.rerun()
else:
    st.sidebar.markdown("**Showing all provinces/states**")
